...
线性表的使用[顺序存储](/notebook/docs/computer/code-design/data-structure/data-basic#顺序存储结构)实现又叫<t c1>顺序表</t>。本节的代码内容是非常简单的，除了掌握代码实现以外，还需要了解C语言指针，内存分配与释放，以及内存如何扩容等知识。
...
- 删除时,从移除位置开始，<t bg1>从左到右依次，向前移动</t>
- 时间复杂度为 $O(n)$

```c
// 核心代码
// deletePos 删除位置
for (int i = deletePos; i < insertPos - 1; i--) {
  L.list[i-1] = L.list[i];
}
```
...
**题目：** 假设利用两个线性表LA和LB分别表示两个集合A和B(即线性表中的数据元素即为集合中的成员)，现要求一个新的集合。
...
- 思路
  - 扩大线性表LA，将存在于线性表LB中而不存在于线性表LA中的数据元素插人到线性表LA中去。
  - 从线性表LB中依次取得每个数据元素，并依值在线性表LA中进行查访，若不存在，则插人LA。
  - 需要注意，<t c2>可以使用线性表ADT</t>来描述算法。（不考虑具体实现）

- 伪代码如下： [点击查看可运行代码](https://onecompiler.com/c/3ydjcajfg)
...
Status Union(SqList* A, SqList B) {
  ...
  for (int i = 0; i < B.length; i++) {
    ElemType b_item;
    GetElem(B, i+1, &b_item)
    // 如果 B中元素不在A中存在
    if (LocateElem(*A, b_item, Compare) == 0) {
      // 将该元素插入A的末尾
      ListInsert(A, (*A).length, b_item);
    }
  }
}
```
...
- 思路
  ...
  - 可设两个指针i 和 j分别指向LA 和 LB中某个元素, k为LC的列表的指针。
  - 比较 $A_i$ 与 $B_j$ 大小
  - 当前 $$C_k = \begin{cases} A_i, A_i >= B_j, i++ \\ B_j, A_i < B_j, j++ \end{cases}$$

- 伪代码 [查看可运行代码](https://onecompiler.com/c/3ydjcajfg)
